...
class FerrisScrapper:
    def __init__(self):
        self._catalogs = self._get_catalogs(
            path=os.path.join(os.getcwd(), "GetAssembly.json")
        )
        self._sections_url = "https://partstream.arinet.com/Parts/GetAssembly?cb=jsonp1722340278493&arib=FRS&aria={aria_id}&includeImgs=true&responsive=true&imgIsThmb=true&arik=FD9iCNGcYKM0XEq1afpU&aril=en-US&ariv=https%253A%252F%252Fwww.ferrislawnmowerparts.com%252Fparts-lookup"
        self._parts_url = "https://partstream.arinet.com/Parts/GetDetails?responsive=true&arik=FD9iCNGcYKM0XEq1afpU&aril=en-US&ariv=https%253A%252F%252Fwww.ferrislawnmowerparts.com%252Fparts-lookup&cb=jsonp1722340278498&ariq={ariq_slug}"
        self._latest_model_code = 136276
        # self._sql_helper = SQLiteHelper("ScagDb.db")
        self._sql_helper = MSSqlHelper()
        self._images = []
    ...
    def _scrape_part(self, ariq_slug: str, sgl_code: str) -> Parts:
        ...
        if response.status_code != 200:
            ...
            return None
        cleaned_response = FerrisScrapper._parse_json_response(response.text)
            ...
            for section in catalog.sections:
                for part in section.parts:
                    record = {
                        ...
                        "sectonDiagram": section.section_diagram,
                        "scraperName": "Ferris",
                    }
